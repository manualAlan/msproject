...
After the individual tests above were completed, a more complete user interface was written so that the useful parts of Phaser could be accessed directly from the ARTIQ dashboard. The final experiment was designed to use both Phaser channels while exposing the main control parameters in a simple experimental interface.
The full source listing for this interface is included in Appendix~\ref{app:multiseq} so that the implementation can be read alongside the experimental discussion in this chapter.
